...
class Drawing extends StateNode {
  ...
  getIsClosed(segments, size) {
    ...
    return firstPoint !== lastPoint && this.currentLineLength > strokeWidth * 4 && Vec.Dist(firstPoint, lastPoint) < strokeWidth * 2;
  }
  startShape() {
    const {
  ...
  updateShapes() {
    ...
    switch (this.segmentMode) {
      case "starting_straight": {
        ...
        const hasMovedFarEnough = Vec.Dist(pagePointWhereNextSegmentChanged, inputs.currentPagePoint) > DRAG_DISTANCE;
        ...
        if (shouldSnap) {
          if (newSegments.length > 2) {
            ...
            for (let i = 0, n = segments.length - 2; i < n; i++) {
              ...
              const distance = Vec.Dist(nearestPointOnSegment, newPoint2);
