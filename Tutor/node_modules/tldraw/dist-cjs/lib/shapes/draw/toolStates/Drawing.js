...
class Drawing extends import_editor.StateNode {
  ...
  getIsClosed(segments, size) {
    ...
    return firstPoint !== lastPoint && this.currentLineLength > strokeWidth * 4 && import_editor.Vec.Dist(firstPoint, lastPoint) < strokeWidth * 2;
  }
  startShape() {
    const {
  ...
  updateShapes() {
    ...
    switch (this.segmentMode) {
      case "starting_straight": {
        ...
        const hasMovedFarEnough = import_editor.Vec.Dist(pagePointWhereNextSegmentChanged, inputs.currentPagePoint) > import_editor.DRAG_DISTANCE;
        ...
        if (shouldSnap) {
          if (newSegments.length > 2) {
            ...
            for (let i = 0, n = segments.length - 2; i < n; i++) {
              ...
              const distance = import_editor.Vec.Dist(nearestPointOnSegment, newPoint2);
  ...
  getLineLength(segments) {
    ...
    for (const segment of segments) {
      for (let i = 0; i < segment.points.length - 1; i++) {
        ...
        length += import_editor.Vec.Sub(B, A).len2();
      }
    }
