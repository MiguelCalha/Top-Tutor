...
class ArrowShapeUtil extends import_editor.ShapeUtil {
  ...
  component(shape) {
    ...
    const info = this.editor.getArrowInfo(shape);
    const bounds = import_editor.Box.ZeroFix(this.editor.getShapeGeometry(shape).bounds);
    ...
    if (onlySelectedShape === shape && shouldDisplayHandles) {
      const sw = 2;
      const { strokeDasharray: strokeDasharray2, strokeDashoffset: strokeDashoffset2 } = (0, import_getPerfectDashProps.getPerfectDashProps)(this.getLength(shape), sw, {
        end: "skip",
        start: "skip",
        lengthRatio: 2.5
      });
  ...
  toSvg(shape, ctx) {
    ...
    const path = getArrowSvgPath(
      info.isStraight ? (0, import_arrowpaths.getSolidStraightArrowPath)(info) : (0, import_arrowpaths.getSolidCurvedArrowPath)(info),
      color,
      strokeWidth
    );
    const { strokeDasharray, strokeDashoffset } = (0, import_getPerfectDashProps.getPerfectDashProps)(
