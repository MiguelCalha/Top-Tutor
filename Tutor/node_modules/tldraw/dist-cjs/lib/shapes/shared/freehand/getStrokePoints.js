...
function getStrokePoints(rawInputPoints, options = {}) {
  ...
  let pt = pts[1];
  while (pt) {
    if (import_editor.Vec.Dist(pt, pts[0]) > size / 3)
      break;
  ...
  pt = pts[pts.length - 1];
  while (pt) {
    if (import_editor.Vec.Dist(pt, last) > size / 3)
      break;
    pts.pop();
    pt = pts[pts.length - 1];
  ...
  pts.push(last);
  const isComplete = options.last || !options.simulatePressure || pts.length > 1 && import_editor.Vec.Dist(pts[pts.length - 1], pts[pts.length - 2]) < size || pointsRemovedFromNearEnd > 0;
